...
1. ##### Windows系統
   ...
   2. 創建需要的image
      api image:
      `docker build -t schedule:2.5.3 -f deploy/api/windows/Dockerfile .`
      migrate image:
   ...
   3. 創建並啟動container
      ...
      2. run api container
         ...
         `docker run --name schedule -p 5487:5487 -e DB_HOST=host.docker.internal -v ${PWD}/docker/log:/app/log schedule:2.5.3`
2. ##### Linux系統
   ...
   2. 創建需要的image
      api image:
      `docker build -t schedule:2.5.3 -f deploy/api/linux/Dockerfile .`
      migrate image:
   ...
   3. 創建並啟動container
      ...
      2. run api container
         ...
         `docker run --name schedule -p 5487:5487 --network="host" -v ${PWD}/docker/log:/app/log schedule:2.5.3`
         `docker run --name schedule -p 5487:5487 -e DB_HOST=192.168.1.11 -e INFLUXDB_HOST=192.168.1.11 -e REDIS_HOST=192.168.1.11 -v ${PWD}/docker/log:/app/log schedule:2.5.3`
