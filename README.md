...
1. ##### Windows系統
   ...
   2. 創建需要的image
      api image:
      `docker build -t schedule:2.13.14 -f deploy/api/windows/Dockerfile .`
      migrate image:
...
2. ##### Linux系統
   ...
   2. 創建需要的image
      api image:
      `docker build -t schedule:2.13.14 -f deploy/api/linux/Dockerfile .`
      migrate image:
